...
@Serializable
private data class SampleClassWithNestedClass(
    val testString: String,
    val testInt: Int,
    val testBoolean: Boolean,
...
private external interface ExtSampleClass : JsAny {
    var testString: String
    var testInt: Int
    var testBoolean: Boolean
}

private external interface ExtNestedClass : JsAny {
    var testInt: Int?
}

private external interface ExtSampleClassWithNestedClass : JsAny {
    var testString: String
    var testInt: Int
    var testBoolean: Boolean
    var testNested: ExtNestedClass
    var testSample: ExtSampleClass
    var testSampleList: JsArray<out ExtSampleClass>
    var testSampleMap: JsMap<JsString, out ExtSampleClass>
    var extraBytes: Int8Array<ArrayBuffer>
    var secondNested: ExtNestedClass?
}
...
open class SerializeBenchmarks {
    ...
    fun benchmarkMsgpackJs() {
        val instance = unsafeJso<ExtSampleClassWithNestedClass> {
            testString = "testString"
            testInt = 10
            testBoolean = true
            testNested = unsafeJso {
                testInt = 5
            }
            testSample = unsafeJso {
                testString = "testString2"
                testInt = 17
                testBoolean = false
            }
            testSampleList = jsArrayOf(unsafeJso {
            ...
            testSampleMap = JsMap(
                jsArrayOf(
                    tupleOf("testString5".toJsString(), unsafeJso {
                    testString = "testString5"
                    testInt = 12
                    testBoolean = false
                }), tupleOf("testString6".toJsString(), unsafeJso {
                    testString = "testString6"
                    testInt = 15
                    testBoolean = true
                })
                )
            )
            extraBytes = byteArrayOf(0x12, 0x13, 0x14, 0x15).toInt8Array()
            secondNested = unsafeJso {
                testInt = null
            }
        }
        encodeMsgpack(instance)
    }
}
